...
const ERROR_OOB = "ERROR: Please enter a number between 0 and 47";

const printFibo = function(){
  let index = +INDEX.value + 1; // Get value from input as number
  if(index >= 1 && index <= 48){ // Make sure that max unsigned int not exceeded
    ...
    alert(ERROR_OOB);
    console.log("You entered: (" + (index - 1) + ") which is not between 0 and 47");
  }
}
